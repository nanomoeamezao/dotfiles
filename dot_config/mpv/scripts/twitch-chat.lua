...
local function load_twitch_chat(is_new_session)
	...
	local sp_ret = mp.command_native({
		...
		args = {
			...
			"POST",
			"--header",
			"Client-ID: " .. o.twitch_client_id,
			"--data",
			utils.format_json(request_body),
			"--silent",
			TWITCH_GRAPHQL_URL,
		},
	})
	...
	for _, curr_comment in ipairs(comments) do
		...
		local banned = false
		for _, v in ipairs(o.banned) do
			if curr_comment_node.commenter.displayName == v then
				banned = true
				break
			end
		end
	...
	elseif sid and not timer then
		if not twitch_video_id then
			...
			if sub_filename then
				twitch_video_id, twitch_client_id_from_track =
					sub_filename:match("https://api%.twitch%.tv/v5/videos/(%d+)/comments%?client_id=(%w+)")
