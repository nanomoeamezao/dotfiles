...
function layout()
	...
	-- deadzone above OSC
	local sh_area_y0, sh_area_y1
	sh_area_y0 = get_align(-1 + (2 * user_opts.deadzonesize), posY - osc_geo.h, 0, 0)
	...
	lo = add_layout("title")
	lo.geometry = geo
	lo.style = string.format("%s{\\clip(0,%f,%f,%f)}", osc_styles.vidTitle, geo.y - geo.h, geo.x + geo.w, geo.y + geo.h)
	lo.alpha[3] = 0
	...
	lo.style = (#tracks_osc.audio > 0) and osc_styles.seekbarFg or osc_styles.seekbarBg
	lo.slider.gap = 3
	lo.slider.tooltip_style = osc_styles.tooltip
	lo.slider.tooltip_an = 2
...
function validate_user_opts()
	...
	if user_opts.volumecontrol ~= "linear" and user_opts.volumecontrol ~= "log" then
		msg.warn('volumecontrol cannot be "' .. user_opts.volumecontrol .. '". Ignoring.')
